...
class TaskChatbot:
    ...
    def parse_message(self, message):
        ...
        return {
            ...
            'planned_start_at': planned_start_at.isoformat() if planned_start_at else None,
        }
    
    def _extract_title(self, message):
        """Extract title từ message"""
        # Xóa các từ khóa thời gian, priority
        title = message
        
        # Xóa các pattern thời gian
        patterns = [
    ...
    def generate_response(self, task_data, prediction=None):
        ...
        if task_data.get('due_at'):
            due = datetime.fromisoformat(task_data['due_at'].replace('Z', '+00:00'))
            response += f"📅 Deadline: {due.strftime('%d/%m/%Y %H:%M')}\n"
        
        if prediction:
            if prediction.get('on_time_prediction') == 1:
                response += f"\n🎯 AI dự đoán: Có thể hoàn thành đúng hạn ({int(prediction.get('confidence', 0)*100)}% tin cậy)"
            else:
                response += f"\n⚠️ AI dự đoán: Có nguy cơ trễ hạn ({int(prediction.get('confidence', 0)*100)}% tin cậy)"
